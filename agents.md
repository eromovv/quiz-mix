...
- **accessToken** — для участников, передаётся в ссылке `?t=...`; только чтение и прохождение квиза.
- **ownerToken** — для организатора; редактирование карточек и продление ссылки.
- Сроки: `ROOM_ACCESS_TOKEN_TTL_HOURS` (по умолчанию 168), `ROOM_OWNER_TOKEN_TTL_HOURS` (720). Логика TTL — `api/room-auth.js`.
