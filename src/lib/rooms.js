...
export async function fetchRoom(roomId, token) {
  ...
  const payload = await readJsonResponse(await fetch(requestUrl));
  return payload;
}
...
export function makeRoomShareUrl(roomId, accessToken) {
  if (typeof window === "undefined") {
  ...
  const query = accessToken ? `?t=${encodeURIComponent(accessToken)}` : "";
  return `${window.location.origin}${base}/room/${roomId}${query}`;
}
