...
export function establishUserSession(roomId, role, room) {
  const session = {
    roomId,
    role,
    expiresAt: role === "owner" ? room.ownerTokenExpiresAt || "" : room.accessTokenExpiresAt || "",
...
export async function loginWithCredentials(roomId, token) {
  ...
  rememberRoomCredentials(normalizedRoomId, {
    ownerToken: role === "owner" ? normalizedToken : "",
    accessToken: payload.accessToken || (role === "guest" ? normalizedToken : ""),
    room: payload.room,
  });
